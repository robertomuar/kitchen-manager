...
<head>
    ...
    <style>
        ...
        th { background-color: #f3f4f6; font-size: 11px; text-align: left; }
        td { font-size: 11px; }
...
<body>
    ...
    <p class="meta">
        Usuario: {{ $user->name ?? $user->email }}<br>
                ...
                @foreach ($items as $item)
                    @php
                        ...
                        if ($expires && !($expires instanceof \Illuminate\Support\Carbon)) {
                            try { $expires = \Illuminate\Support\Carbon::parse($expires); } catch (\Throwable $e) { $expires = null; }
                        }
                    @endphp
                    <tr>
